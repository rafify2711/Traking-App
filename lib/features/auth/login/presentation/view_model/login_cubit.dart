  ...
  Future<void> login({required String email, required String password}) async {
    ...
    switch(result){
      case ApiSuccess<LoginResponse>():{
        emit(state.copyWith(loginState: BaseSuccess<LoginResponse>(data: result.data)));

      }
      case ApiError<LoginResponse>():{
        emit(state.copyWith(loginState: BaseError<LoginResponse>(errorMessage: result.failure?.errorMessage ?? "Something went wrong")));
